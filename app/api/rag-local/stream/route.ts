...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Processing Local RAG streaming query: ${query}`);
    ...
    const searchResults = await localRAGSystem.search(query, options);
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          // Stream the response
          const responseStream = localRAGSystem.generateStreamingResponse(query, searchResults, options);
    ...
    console.error('Local RAG streaming setup error:', error);
